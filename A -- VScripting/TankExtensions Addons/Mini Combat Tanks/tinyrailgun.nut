local TINYCOMBATTANK_VALUES_TABLE = {
	TINYCOMBATTANK_RAILGUN_SND_CHARGE           = ")misc/doomsday_cap_close_quick.wav"
	TINYCOMBATTANK_RAILGUN_SND_CHARGE2          = ")misc/doomsday_cap_down.wav"
	TINYCOMBATTANK_RAILGUN_SND_FIRE1            = ")weapons/shooting_star_shoot_charged.wav"
	TINYCOMBATTANK_RAILGUN_SND_FIRE2            = ")weapons/sniper_railgun_charged_shot_crit_01.wav"
	TINYCOMBATTANK_RAILGUN_SND_COOL_START       = ")misc/doomsday_cap_close_start.wav"
	TINYCOMBATTANK_RAILGUN_SND_COOL_END         = ")misc/doomsday_cap_spin_start.wav"
	TINYCOMBATTANK_RAILGUN_MODEL                = "models/bots/boss_bot/combat_tank_mk2/mk2_railgun.mdl"
	TINYCOMBATTANK_RAILGUN_MODEL_CASING         = "models/weapons/shells/shell_sniperrifle.mdl"
	TINYCOMBATTANK_RAILGUN_PARTICLE_TRACER_RED  = "dxhr_sniper_rail_red"
	TINYCOMBATTANK_RAILGUN_PARTICLE_TRACER_BLUE = "dxhr_sniper_rail_blue"
	TINYCOMBATTANK_RAILGUN_COOLDOWN_TIME        = 8
	TINYCOMBATTANK_RAILGUN_BULLET_DAMAGE        = 100
	TINYCOMBATTANK_RAILGUN_CLIP_SIZE            = 2
}
foreach(k,v in TINYCOMBATTANK_VALUES_TABLE)
	if(!(k in TankExt.ValueOverrides))
		ROOT[k] <- v

PrecacheModel(TINYCOMBATTANK_RAILGUN_MODEL)
PrecacheModel(TINYCOMBATTANK_RAILGUN_MODEL_CASING)
TankExt.PrecacheSound(TINYCOMBATTANK_RAILGUN_SND_CHARGE)
TankExt.PrecacheSound(TINYCOMBATTANK_RAILGUN_SND_CHARGE2)
TankExt.PrecacheSound(TINYCOMBATTANK_RAILGUN_SND_FIRE1)
TankExt.PrecacheSound(TINYCOMBATTANK_RAILGUN_SND_FIRE2)
TankExt.PrecacheSound(TINYCOMBATTANK_RAILGUN_SND_COOL_START)
TankExt.PrecacheSound(TINYCOMBATTANK_RAILGUN_SND_COOL_END)

TankExt.TinyCombatTankWeapons["tinyrailgun"] <- {
	function SpawnModel()
	{
		local hRailgun = CreateByClassnameSafe("funCBaseFlex")
		hRailgun.SetModel(TINYCOMBATTANK_RAILGUN_MODEL)
	...
	function OnSpawn()
	{
		...
		local iClip         = TINYCOMBATTANK_RAILGUN_CLIP_SIZE
		...
		local hCasingShooter = SpawnEntityFromTableSafe("env_shooter", {
			...
			shootmodel       = TINYCOMBATTANK_RAILGUN_MODEL_CASING
			m_flGibLife      = 5
			m_flVariance     = 0.15
			m_flVelocity     = 200
			m_iGibs          = 1
		...
		{
			iClip--
			flNextAttack = Time() + (iClip == 0 ? TINYCOMBATTANK_RAILGUN_COOLDOWN_TIME : 0.7)
			SetLaserColor(null)
			bCharging = false
			hTank_scope.AddToSoundQueue({
				sound_name  = TINYCOMBATTANK_RAILGUN_SND_FIRE1
				pitch       = 65
				sound_level = 100
				entity      = hTank
			...
			hTank_scope.AddToSoundQueue({
				sound_name  = TINYCOMBATTANK_RAILGUN_SND_FIRE2
				pitch       = 90
				sound_level = 100
				entity      = hTank
				filter_type = RECIPIENT_FILTER_GLOBAL
			...
			if("enthit" in LaserTrace)
			{
				...
				if(hHit.IsValid() && hHit.GetTeam() != iTeamNum)
				{
					...
					hHit.TakeDamageCustom(hTank, hTank, null, Vector(), Vector(), TINYCOMBATTANK_RAILGUN_BULLET_DAMAGE, DMG_BULLET | DMG_ACID, TF_DMG_CUSTOM_HEADSHOT)
				}
			}
			...
			local hParticle = SpawnEntityFromTableSafe("info_particle_system", {
				origin       = vecBarrel
				effect_name  = (bBlueTeam ? TINYCOMBATTANK_RAILGUN_PARTICLE_TRACER_BLUE : TINYCOMBATTANK_RAILGUN_PARTICLE_TRACER_RED)
				start_active = 1
			})
			...
			TankExt.DelayFunction(self, this, 0.1, function()
			{
		...
		function TinyCombatTankWeaponThink()
		{
			...
			if(!bCharging && bInLaser && bCanAttack && flCoolPercent <= 0)
			{
				...
				bCharging = true
				hTank_scope.AddToSoundQueue({
					sound_name  = TINYCOMBATTANK_RAILGUN_SND_CHARGE
					sound_level = 100
					entity      = hTank
					filter_type = RECIPIENT_FILTER_GLOBAL
				})
				hTank_scope.AddToSoundQueue({
					sound_name  = TINYCOMBATTANK_RAILGUN_SND_CHARGE2
					sound_level = 100
					entity      = hTank
					filter_type = RECIPIENT_FILTER_GLOBAL
				})
				self.ResetSequence(self.LookupSequence("fire"))
				if(iClip == 0) iClip = TINYCOMBATTANK_RAILGUN_CLIP_SIZE
			...
			if(flCoolPercentLast == 0 && flCoolPercent > 0)
				hTank_scope.AddToSoundQueue({
					sound_name  = TINYCOMBATTANK_RAILGUN_SND_COOL_START
					sound_level = 85
					pitch       = 115
					entity      = hTank
					filter_type = RECIPIENT_FILTER_GLOBAL
			...
			if(flCoolPercentLast == 1 && flCoolPercent < 1)
				hTank_scope.AddToSoundQueue({
					sound_name  = TINYCOMBATTANK_RAILGUN_SND_COOL_END
					sound_level = 85
					pitch       = 115
					entity      = hTank
					filter_type = RECIPIENT_FILTER_GLOBAL
