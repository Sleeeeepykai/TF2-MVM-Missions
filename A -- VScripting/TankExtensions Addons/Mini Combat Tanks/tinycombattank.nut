local TINYCOMBATTANK_VALUES_TABLE = {
	TINYCOMBATTANK_SND_ROTATE           = ")plats/tram_move.wav"
	TINYCOMBATTANK_SND_UBER             = "player/invulnerable_on.wav"
	TINYCOMBATTANK_SND_UBER_OFF         = "player/invulnerable_off.wav"
	TINYCOMBATTANK_ROTATE_SPEED_DEFAULT = 0.8
	TINYCOMBATTANK_POSE_YAW             = 2
	TINYCOMBATTANK_POSE_PITCH           = 1
	TINYCOMBATTANK_MODEL                = "models/bots/boss_bot/combat_tank_mk2/mk2_combat_tank_chassis.mdl"
	TINYCOMBATTANK_MODEL_UBERED_TRACK_L = "models/bots/boss_bot/ubertank/tank_uber_track_l.mdl"
	TINYCOMBATTANK_MODEL_UBERED_TRACK_R = "models/bots/boss_bot/ubertank/tank_uber_track_r.mdl"
	TINYCOMBATTANK_TRACK_L_RED          = "models/bots/boss_bot/tankred_track_l.mdl"
	TINYCOMBATTANK_TRACK_R_RED          = "models/bots/boss_bot/tankred_track_r.mdl"
	TINYCOMBATTANK_MAX_RANGE            = 1200
}
foreach(k,v in TINYCOMBATTANK_VALUES_TABLE)
	if(!(k in TankExt.ValueOverrides))
		ROOT[k] <- v

PrecacheModel(TINYCOMBATTANK_MODEL)
PrecacheModel(TINYCOMBATTANK_TRACK_L_RED)
PrecacheModel(TINYCOMBATTANK_TRACK_R_RED)
PrecacheModel(TINYCOMBATTANK_MODEL_UBERED_TRACK_L)
PrecacheModel(TINYCOMBATTANK_MODEL_UBERED_TRACK_R)
TankExt.PrecacheSound(TINYCOMBATTANK_SND_ROTATE)
TankExt.PrecacheSound(TINYCOMBATTANK_SND_UBER)
TankExt.PrecacheSound(TINYCOMBATTANK_SND_UBER_OFF)

::TinyCombatTankEvents <- {
	function OnGameEvent_recalculate_holidays(_) { if(GetRoundState() == 3) delete ::TinyCombatTankEvents }
	function OnScriptHook_OnTakeDamage(params)
	{
		...
		{
			local CombatScope = TankExt.GetMultiScopeTable(hVictim.GetScriptScope(), "tinycombattank")
...
__CollectGameEventCallbacks(TinyCombatTankEvents)

TankExt.TinyCombatTankWeapons <- {}

TankExt.NewTankType("tinycombattank*", {

	Model = TINYCOMBATTANK_MODEL
	Scale = 0.5
	DisableSmokestack = 1
	DisableBomb = 1
	MaxStepHeight = 50
	...
	function OnSpawn()
	{
		...
		local flRotateSpeed = TINYCOMBATTANK_ROTATE_SPEED_DEFAULT
		local iLaser        = self.LookupAttachment("laser_origin")
		vecMountOffset <- Vector(0, 0, 55)
		vecMount       <- null
		vecTarget      <- null
		hTarget        <- null
		flDist         <- TINYCOMBATTANK_MAX_RANGE
		flAngleDot     <- -1
		...
		EmitSoundEx({
			sound_name  = TINYCOMBATTANK_SND_ROTATE
			sound_level = 85
			entity      = self
		...
		if(!bBlueTeam) TankExt.SetTankModel(self, {
			LeftTrack  = TINYCOMBATTANK_TRACK_L_RED
			RightTrack = TINYCOMBATTANK_TRACK_R_RED
		})
		self.SetSkin(bBlueTeam ? bFinalSkin ? 3 : 2 : bFinalSkin ? 1 : 0)
		local bHasTurretReplacement = false
		for(local i = 1; i <= 2; i++)
			if(sParams[i] in TankExt.TinyCombatTankWeapons)
			{
				local WeaponTable = TankExt.TinyCombatTankWeapons[sParams[i]]
				local hWeapon
				...
				{
					if(i > 1)
						ClientPrint(null, HUD_PRINTTALK, format("\x07FFFF00TinyCombatTank weapon \"%s\" does not support additional weapons", sParams[i]))
			...
			else ClientPrint(null, HUD_PRINTTALK, format("\x07FFFF00TinyCombatTank weapon \"%s\" is invalid, weapon is not loaded or does not exist", sParams[i]))
		...
		function ToggleUber()
		{
			if(!bUberFizzle)
				if(!bUbered)
				{
					bUbered = true
					EmitSoundEx({
						sound_name  = TINYCOMBATTANK_SND_UBER
						filter_type = RECIPIENT_FILTER_GLOBAL
					})
					LastSkins.clear()
					...
					SetPropIntArray(hTrackL, "m_nModelIndexOverrides", GetModelIndex(TINYCOMBATTANK_MODEL_UBERED_TRACK_L), 0)
					SetPropIntArray(hTrackL, "m_nModelIndexOverrides", GetModelIndex(TINYCOMBATTANK_MODEL_UBERED_TRACK_L), 3)
					SetPropIntArray(hTrackR, "m_nModelIndexOverrides", GetModelIndex(TINYCOMBATTANK_MODEL_UBERED_TRACK_R), 0)
					SetPropIntArray(hTrackR, "m_nModelIndexOverrides", GetModelIndex(TINYCOMBATTANK_MODEL_UBERED_TRACK_R), 3)
				...
				else
				{
					bUberFizzle = true
					EmitSoundEx({
						sound_name  = TINYCOMBATTANK_SND_UBER_OFF
						filter_type = RECIPIENT_FILTER_GLOBAL
					})
		...
		function Think()
		{
			...
			local hForceTarget = FindByNameNearest("tinycombattank_target", vecMount, flDist)
			if(hForceTarget)
			{
			...
			if(hTarget)
			{
				...
				flRotateSpeed = TINYCOMBATTANK_ROTATE_SPEED_DEFAULT
			...
			else
			{
				...
				{
					flRotateSpeed = TINYCOMBATTANK_ROTATE_SPEED_DEFAULT * 0.5
			...
			self.SetPoseParameter(TINYCOMBATTANK_POSE_PITCH, TankExt.Clamp(angCurrent.x, -14, 14))
			...
			self.SetPoseParameter(TINYCOMBATTANK_POSE_YAW, angCurrent.y)
