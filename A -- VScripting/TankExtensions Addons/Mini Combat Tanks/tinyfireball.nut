local TINYCOMBATTANK_VALUES_TABLE = {
	TINYCOMBATTANK_FIREBALL_BURN_DAMAGE_BONUS  = Convars.GetFloat("tf_fireball_burning_bonus") // 3
	TINYCOMBATTANK_FIREBALL_BURN_DURATION      = Convars.GetFloat("tf_fireball_burn_duration") // 2
	TINYCOMBATTANK_FIREBALL_CONE_RADIUS        = 15
	TINYCOMBATTANK_FIREBALL_DAMAGE             = Convars.GetFloat("tf_fireball_damage") // 25
	TINYCOMBATTANK_FIREBALL_MODEL              = "models/bots/boss_bot/combat_tank_mk2/mk2_fireball.mdl"
	TINYCOMBATTANK_FIREBALL_RECHARGE_TIME      = 1.5
	TINYCOMBATTANK_FIREBALL_SND_FIRE           = "Weapon_DragonsFury.Single"
}
foreach(k,v in TINYCOMBATTANK_VALUES_TABLE)
	if(!(k in TankExt.ValueOverrides))
		ROOT[k] <- v

PrecacheModel(TINYCOMBATTANK_FIREBALL_MODEL)
TankExt.PrecacheSound(TINYCOMBATTANK_FIREBALL_SND_FIRE)
TankExt.PrecacheParticle("projectile_fireball")

::TinyCombatTankFireballEvents <- {
	OnGameEvent_recalculate_holidays = function(_) { if(GetRoundState() == 3) delete ::TinyCombatTankFireballEvents }
	OnScriptHook_OnTakeDamage = function(params)
	{
		local hAttacker = params.attacker
		local hVictim   = params.const_entity
		if(hAttacker && hVictim && hAttacker.GetClassname() == "tinycombattank_fireball")
		{
			// this is horrible
			...
			if(hAttacker.GetTeam() != hVictim.GetTeam() && !(hVictim in hAttacker_scope.HitEntities))
			{
				...
				params.damage          = TINYCOMBATTANK_FIREBALL_DAMAGE
				...
				if(hVictim.IsPlayer())
				{
					...
					params.damage *= (bBonusDamage ? TINYCOMBATTANK_FIREBALL_BURN_DAMAGE_BONUS : 1.0)
					...
					if(hVictim.GetPlayerClass() == TF_CLASS_PYRO) hVictim.AddCondEx(TF_COND_BURNING_PYRO, TINYCOMBATTANK_FIREBALL_BURN_DURATION * 0.5, hOwner)
					local hIgniter = CreateByClassnameSafe("trigger_ignite")
					hIgniter.KeyValueFromInt("spawnflags", 1)
					hIgniter.KeyValueFromInt("burn_duration", TINYCOMBATTANK_FIREBALL_BURN_DURATION)
				...
				{
					bBonusDamage = true
					params.damage *= TINYCOMBATTANK_FIREBALL_BURN_DAMAGE_BONUS
...
__CollectGameEventCallbacks(TinyCombatTankFireballEvents)

TankExt.TinyCombatTankWeapons["tinyfireball"] <- {
	function SpawnModel()
	{
		local hFireball = CreateByClassnameSafe("funCBaseFlex")
		hFireball.SetModel(TINYCOMBATTANK_FIREBALL_MODEL)
	...
	function OnSpawn()
	{
		...
		function TinyCombatTankWeaponThink()
		{
			...
			local flTime       = Time()
			local bEnemyInCone = hTank_scope.flAngleDot >= cos(TINYCOMBATTANK_FIREBALL_CONE_RADIUS * DEG2RAD)
			...
			local function GetEffectBarProgress(bUpper)
			{
				...
				if(flTime < flRechargeTime)
					return (TINYCOMBATTANK_FIREBALL_RECHARGE_TIME - flRechargeTime + flTime) / TINYCOMBATTANK_FIREBALL_RECHARGE_TIME
				return 1.0
			}
			...
			local function ShootAtTarget(vecOrigin)
			{
				...
				hProj.DispatchSpawn()
				SetPropString(hProj, "m_iClassname", "tinycombattank_fireball")
			...
			{
				local flDelta = flTime + TINYCOMBATTANK_FIREBALL_RECHARGE_TIME
				...
				hTank_scope.AddToSoundQueue({
					sound_name  = TINYCOMBATTANK_FIREBALL_SND_FIRE
					sound_level = 84
					entity      = hTank
