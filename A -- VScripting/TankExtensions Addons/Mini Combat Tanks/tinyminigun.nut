local TINYCOMBATTANK_VALUES_TABLE = {
	TINYCOMBATTANK_MINIGUN_SPREAD_MULTIPLIER = 1
	TINYCOMBATTANK_MINIGUN_SND_SPINUP        = ")weapons/minigun_wind_up.wav"
	TINYCOMBATTANK_MINIGUN_SND_SPINNING      = ")weapons/minigun_spin.wav"
	TINYCOMBATTANK_MINIGUN_SND_SPINDOWN      = ")weapons/minigun_wind_down.wav"
	TINYCOMBATTANK_MINIGUN_SND_FIRE          = ")weapons/minigun_shoot.wav"
	TINYCOMBATTANK_MINIGUN_PARTICLE_TRACER   = "bullet_tracer01"
	TINYCOMBATTANK_MINIGUN_PARTICLE_MUZZLE   = "muzzle_minigun_constant"
	TINYCOMBATTANK_MINIGUN_PARTICLE_CASING   = "eject_minigunbrass"
	TINYCOMBATTANK_MINIGUN_MODEL             = "models/bots/boss_bot/combat_tank_mk2/mk2_minigun.mdl"
	TINYCOMBATTANK_MINIGUN_FIRE_DELAY        = 0.105
	TINYCOMBATTANK_MINIGUN_CONE_RADIUS       = 25
	TINYCOMBATTANK_MINIGUN_BULLET_DAMAGE     = 10
}
foreach(k,v in TINYCOMBATTANK_VALUES_TABLE)
	if(!(k in TankExt.ValueOverrides))
		ROOT[k] <- v

PrecacheModel(TINYCOMBATTANK_MINIGUN_MODEL)
TankExt.PrecacheSound(TINYCOMBATTANK_MINIGUN_SND_SPINUP)
TankExt.PrecacheSound(TINYCOMBATTANK_MINIGUN_SND_SPINNING)
TankExt.PrecacheSound(TINYCOMBATTANK_MINIGUN_SND_FIRE)
TankExt.PrecacheSound(TINYCOMBATTANK_MINIGUN_SND_SPINDOWN)

TankExt.TinyCombatTankWeapons["tinyminigun"] <- {
	Model = TINYCOMBATTANK_MINIGUN_MODEL
	DefaultAnim = "idle"
	function OnSpawn()
	{
		local SpawnParticleSystem = @(sParticle) SpawnEntityFromTableSafe("info_particle_system", { effect_name = sParticle })
		local hParticleMuzzle1 = SpawnParticleSystem(TINYCOMBATTANK_MINIGUN_PARTICLE_MUZZLE)
		local hParticleMuzzle2 = SpawnParticleSystem(TINYCOMBATTANK_MINIGUN_PARTICLE_MUZZLE)
		local hParticleCasing1 = SpawnParticleSystem(TINYCOMBATTANK_MINIGUN_PARTICLE_CASING)
		local hParticleCasing2 = SpawnParticleSystem(TINYCOMBATTANK_MINIGUN_PARTICLE_CASING)
		...
		function TinyCombatTankWeaponThink()
		{
			...
			if(hTank_scope.hTarget)
			{
				if(iState == 0)
				{
					...
					hTank_scope.AddToSoundQueue({
						sound_name  = TINYCOMBATTANK_MINIGUN_SND_SPINUP
						volume      = 0.9
						sound_level = 120
						entity      = hTank
				...
				{
					local bEnemyInCone = hTank_scope.flAngleDot >= cos(TINYCOMBATTANK_MINIGUN_CONE_RADIUS * DEG2RAD)
					...
					if(!bEnemyInCone && iState != 2)
					{
						...
						hTank_scope.AddToSoundQueue({
							sound_name  = TINYCOMBATTANK_MINIGUN_SND_SPINNING
							volume      = 0.8
							sound_level = 120
							entity      = hTank
							filter_type = RECIPIENT_FILTER_GLOBAL
					...
					if(bEnemyInCone)
					{
						if(iState != 3)
						{
							...
							hTank_scope.AddToSoundQueue({
								sound_name  = TINYCOMBATTANK_MINIGUN_SND_FIRE
								sound_level = 120
								entity      = hTank
								filter_type = RECIPIENT_FILTER_GLOBAL
							})
						...
						{
							flNextAttack = flTime + TINYCOMBATTANK_MINIGUN_FIRE_DELAY
							iBarrel = iBarrel ? 0 : 1
							local vecTowardsEnemy = RotatePosition(Vector(), QAngle(RandomFloat(-5, 5), RandomFloat(-5, 5)) * TINYCOMBATTANK_MINIGUN_SPREAD_MULTIPLIER, hTank_scope.vecTarget - hTank_scope.vecMount)
							...
							if("enthit" in Trace)
							{
								...
								if("GetTeam" in hHit && hHit.GetTeam() != hTank.GetTeam())
								{
									...
									hHit.TakeDamageCustom(hTank, hTank, null, Vector(), Vector(), TINYCOMBATTANK_MINIGUN_BULLET_DAMAGE, DMG_BULLET, TF_DMG_CUSTOM_MINIGUN)
								}
							}

							local hParticleTracer = SpawnEntityFromTableSafe("info_particle_system", {
								effect_name = TINYCOMBATTANK_MINIGUN_PARTICLE_TRACER
								start_active = 1
							})
			...
			else if(iState != 0)
			{
				iState = 0
				hTank_scope.AddToSoundQueue({
					sound_name  = TINYCOMBATTANK_MINIGUN_SND_SPINDOWN
					volume      = 0.9
					sound_level = 120
					entity      = hTank
					filter_type = RECIPIENT_FILTER_GLOBAL
			...
			if(iState != 0) SoundStop(TINYCOMBATTANK_MINIGUN_SND_SPINDOWN)
			if(iState != 1) SoundStop(TINYCOMBATTANK_MINIGUN_SND_SPINUP)
			if(iState != 2) SoundStop(TINYCOMBATTANK_MINIGUN_SND_SPINNING)
			if(iState != 3) SoundStop(TINYCOMBATTANK_MINIGUN_SND_FIRE)
			return -1
		}
