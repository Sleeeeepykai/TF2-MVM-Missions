printl("ClusterBombLogic loaded. V18")

::ClusterBombLogic <-
{
	// Can fire function after entity dies (i.e. when sticky detonates)
	...
	function SpawnStickyBombs(ent, owner)
	{
		ent.ValidateScriptScope()
		if (!("detonate" in ent.GetScriptScope())) return
		local totalCount = 8; // This is counting the total number of stickies that will be spawned
		local upSpeed = 400; // This is counting how fast they are propelled upwards
		local speed = 800; // This is counting how far they are propelled
		local origin = startOrigin; // This is getting the origin of the big stickybomb

		for (local i = 0; i < totalCount; i++) // This is iterating over all of the stickies that will be spawned
		{
			local yaw = (360 / totalCount) * i; // This is calculating the angle for each sticky to be propelled at

			local dir = QAngle(0, yaw).Forward(); // This is calculating the full direction for each sticky to be propelled at (aka gets a forward vector from a qangle)
			dir.Norm(); // This is normalising the direction, or setting its magnitude to 1 so it doesn't affect velocity calculations and keeps velocity consistent
			local velocity = dir * speed;
			velocity.z += upSpeed;

			local projectile = Entities.CreateByClassname("tf_projectile_rocket");
			projectile.SetTeam(owner.GetTeam());
			NetProps.SetPropEntity(projectile, "m_hThrower", owner);

			projectile.DispatchSpawn();
			projectile.SetAbsOrigin(origin + Vector(0,0,15) + dir*5);
			projectile.SetAbsVelocity(velocity);
			projectile.KeyValueFromString("classname", "clustersticky");
			printl(projectile.tostring() + " spawned")
		}
	}
	function BotTagCheck(player)
	{
		if (player.HasBotTag("clusterbombdemo"))
		{
			player.ValidateScriptScope() // This is creating a scope for the robot so we can handle things with just the robot
			local scope = player.GetScriptScope()
			scope.Think <- function () // Creating a think function in the robot's scope
			{
				for (local ent; ent = Entities.FindByClassname(ent, "tf_projectile_pipe_remote");) // Setting a for loop to iterate over all stickies
				{
					if (NetProps.GetPropEntity(ent, "m_hThrower") == self) // Checks if the stickies belong to the robot
					{
						ent.ValidateScriptScope();
						local proj_scope = ent.GetScriptScope();

						if ("marked" in proj_scope) continue
						proj_scope.marked <- 0;
						ent.SetModelScale(2, 0); // This doesn't seem to affect the stickybomb?
						proj_scope.startTime <- Time()

						ClusterBombLogic.SetDestroyCallback(ent, function() {
							ClusterBombLogic.SpawnStickyBombs(self, self.GetOwner())
						})
					}
				}
			}
			AddThinkToEnt(player, "Think"); // This is looking for a function named "Think" in the scope of the player or in the global scope, and then setting it to be the think function of the player
			printl("ClusterBombLogic added to bot.");
		}
	}
	function OnGameEvent_demoman_det_stickies(params)
	{
		local player = GetPlayerFromUserID(params.player)
		if (!player || !player.HasBotTag("clusterbombdemo")) return
		local best = [null, 0];
		for (local ent; ent = Entities.FindByClassname(ent, "tf_projectile_pipe_remote");)
		{
			if (!ent.GetOwner().IsValid() || ent.GetOwner() != player) continue
			ent.ValidateScriptScope()

			lifetime = Time() - ent.GetScriptScope().startTime
			if (best[1] < lifetime) best = [ent, lifetime]
		}
		if (best[0]) ClusterBombLogic.SpawnStickyBombs(best[0].GetOrigin(), player)
	}
	function OnScriptHook_OnTakeDamage(params)
	{
		...
		if (!ent || !ent.IsValid || damage <= 0) return
		ent.ValidateScriptScope()
	...
	function OnGameEvent_player_spawn(params)
	{
		...
		if (player == null || !player.IsValid() || !player.IsBotOfType(1337))
			return

		EntFireByHandle(player, "RunScriptCode", "ClusterBombLogic.BotTagCheck(self)", -1.0, null, null)
	}
}

__CollectGameEventCallbacks(ClusterBombLogic)
...
//is to lose custody and the land is to be assigned to two law-worthy and discreet men of that fee who
...
//(15) No town or free man is to be distrained to make bridges or bank works save for those that ought
//to do so of old and by right.
...
//debt, by view of law-worthy men, so that nothing is to be removed thence until the debt that remains
//is paid to us, and the remainder is to be released to the executors to discharge the will of the dead
//man, and if nothing is owed to us from such a person, all the chattels are to pass to the (use of) the
//dead man, saving to the dead man’s wife and children their reasonable portion.

//(19) No constable or his bailiff is to take corn or other chattels from anyone who not themselves of a
//vill where a castle is built, unless the constable or his bailiff immediately offers money in payment of
//obtains a respite by the wish of the seller. If the person whose corn or chattels are taken is of such a
//vill, then the constable or his bailiff is to pay the purchase price within forty days.
...
//or knight or any other lady (sic) is to be taken by our bailiffs, nor will we or our bailiffs or anyone
//else take someone else’s timber for a castle or any other of our business save by the will of he to
//whom the timber belongs.
...
//(24) The writ called ‘praecipe’ is not to be issued to anyone in respect to any free tenement in such a
//way that a free man might lose his court.

//(25) There is to be a single measure for wine throughout our realm, and a single measure for ale, and
//a single measure for Corn, that is to say the London quarter, and a single breadth for dyed cloth,
//russets, and haberjects, that is to say two yards within the lists. And it shall be the same for weights
//as for measures.

//(26) Henceforth there is to be nothing given for a writ of inquest from the person seeking an inquest
//of life or member, but such a writ is to be given freely and is not to be denied.
...
//baron held them, nor, on account of such a barony or escheat, are we to have the escheat or custody
//of any of our men unless the man who held the barony or the escheat held elsewhere from us in chief.
...
//(34) No-one is to be taken or imprisoned on the appeal of woman for the death of anyone save for the
//death of that woman’s husband.

//(35) No county court is to be held save from month to month, and where the greater term used to be
...
//fter Michaelmas, and the view of frankpledge is to be taken at the Michaelmas term without
...
//Hereford, R(anulf) bishop of Chichester, W(illiam) bishop of Exeter, the abbot of (Bury) St
...
//Vieuxpont, W(illiam) Brewer, R(ichard) de Montfiquet, P(eter) fitz Herbert, W(illiam) de Aubigné,
//G. Gresley, F. de Braose, J(ohn) of Monmouth, J(ohn) fitz Alan, H(ugh) de Mortemer, W(illiam) de
//Beauchamp, W(illiam) de St John, P(eter) de Maulay, Brian de Lisle, Th(omas) of Moulton,
//R(ichard) de Argentan, G(eoffrey) de Neville, W(illiam) Mauduit, J(ohn) de Baalon and others.
...
//ourselves and our heirs that the aforesaid charter is to be firmly and inviably observed in all and each
//of its articles in perpetuity, including any articles contained in the same charter which by chance
//have not to date been observed. In testimony of which we have had made these our letters patent.
//Witnessed by Edward our son, at Westminster on the twelfth day of October in the twenty-fifth year
//of our reign. (Chancery warranty by John of) Stowe.

//callate puta de india @test callate puta de india @test callate puta de india @test callate puta de india @test callate puta de india